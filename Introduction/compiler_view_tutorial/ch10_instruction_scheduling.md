...
在编译器设计领域，指令调度是将中间表示（IR）转化为高效可执行代码的关键环节。对于 PyTorch Inductor 而言，调度（Scheduling）不仅决定了 kernel 的执行顺序，还决定了哪些操作可以被融合（fusion）、哪些缓冲区可以被复用、以及最终生成的代码在 GPU 上的运行效率。
...
> **注：** 代码生成（codegen）在调度器初始化完成后，由 `GraphLowering.codegen()` 单独调用，不属于调度管线本身。

### 10.6 数据依赖与控制依赖 (Data and Control Dependencies)

#### 10.6.1 原理

依赖关系是调度的核心约束。编译器理论中定义了三种基本数据依赖类型：RAW（Read After Write，真依赖）、WAR（Write After Read，反依赖）和 WAW（Write After Write，输出依赖），分别对应操作间的数据流动、读取保护和写入顺序约束。此外还有控制依赖，指操作的执行取决于前序操作的结果。详细定义见第 6 章 2.1 节和 EaC Section 11.2。
...
列表调度（EaC Section 11.3）是最经典的启发式调度算法。核心思想是为每个操作计算优先级（如关键路径长度），维护一个"就绪列表"（所有依赖已满足的操作），然后按优先级从就绪列表中选择操作执行。前向列表调度从入口节点开始，后向列表调度从出口节点反向进行。完整算法描述见 EaC Ch.11。
...
其中 `min_input_distance` 是节点在依赖图中的最短路径深度（保守估计），`max_input_distance` 是最长路径深度（接近最早开始时间 EST）。两者用途不同：后者用于关键路径分析，前者用于其他启发式。
...
### 10.10 Tiling-Based Reduction Splitting

#### 10.10.1 原理

软件流水线（EaC Section 11.6）是一种循环调度技术，通过重叠不同迭代的操作来利用指令级并行。与之不同，Inductor 的 `split_reduction` 采用的是**基于分块的归约分割**（tiling-based reduction splitting）：将一个大的 reduction 沿某个维度切分为多个 tile，每个 tile 独立进行部分归约，最后合并结果。这不是软件流水线——软件流水线涉及迭代间的重叠执行，而分块归约是将工作划分为独立子任务。

#### 10.10.2 为什么需要

大型 reduction 操作可能超出单个 kernel 的计算或存储能力。通过分块，可以控制每个 kernel 的工作集大小，提高 GPU occupancy 和寄存器利用率。

#### 10.10.3 在 Inductor 中的体现

`MixOrderReduction` 类实现了跨维度的归约融合——将沿不同维度归约同一输入的两个 reduction 节点融合为一个 kernel。`split_reduction` 相关逻辑将大的 reduction 按维度切分为多个块，每个块独立处理。
...
而 `GroupedSchedulerNode` 的设计目的是保证一组节点不被打断调度（类似于原子调度单元），主要用于 foreach 算子和 collective 操作。它不允许外部节点插入其中，也不允许外部节点与组内节点融合：
...
拓扑排序是将 DAG 的节点排成线性序列，使得每条边 $(u, v)$ 中 $u$ 出现在 $v$ 之前。两种经典算法：Kahn 算法（BFS，按入度逐层处理）和 DFS 后序遍历，时间复杂度均为 $O(|V| + |E|)$。Inductor 两者都用：`_topological_sort_nodes()` 使用 Kahn 算法并按层分组返回（便于后续融合分析），`topological_sort_schedule()` 使用 DFS（用于融合后恢复拓扑顺序）。
...
1. **调度粒度**：LLVM 调度单条机器指令以利用指令级并行（ILP），Inductor 调度整个 GPU kernel，关注融合和内存优化
2. **依赖模型**：LLVM 使用寄存器级依赖，Inductor 使用 Buffer + 索引级依赖（`MemoryDep` 精确到索引表达式）
3. **调度与分配的关系**：LLVM 的指令调度在寄存器分配之前，Inductor 的调度在更高级别，寄存器分配由 Triton 后端自动管理
...
### 10.25 Eager-first 哲学：调度如何保持 eager 模式语义
