...
**原理**（*Engineering a Compiler* Ch.8-9）

数据依赖描述的是两条语句之间的执行顺序约束。当两条语句操作同一内存位置时，它们之间就存在数据依赖。编译器需要精确地识别这些依赖关系，才能进行安全的代码变换（如指令调度、循环变换、算子融合）。
...
控制依赖描述的是：如果谓词 P 决定了语句 S 是否执行，则 S 控制依赖于 P。形式化定义：从程序入口到 S 的每条路径都经过 P，且存在从 P 到 S 的路径使得 P 的结果决定了这条路径是否到达 S。

**为什么需要**

在传统编译器中，控制依赖决定了哪些语句可以移动到条件分支之外（if-conversion、谓词执行）。违反控制依赖会改变程序语义。

**在 Inductor 中的体现**

Inductor 在调度阶段不显式处理控制依赖。原因：
1. FX graph 已经是 dataflow 形式（没有显式控制流）
2. `torch.cond` 等控制流在 Dynamo 阶段就被处理为 graph break 或 specialized
3. 调度器假设所有节点都需要执行（DCE 除外），依赖关系纯粹由数据依赖决定

唯一隐式的"控制"概念是 unbacked symbol 的依赖：当某个节点定义了一个运行时才能确定的符号整数值（如 `nonzero` 的输出大小），后续使用该符号的节点必须排在其后。这在 `compute_dependencies()` 中通过 `unbacked_symbol_to_origin_node` 字典跟踪（`scheduler.py` line 3568-3619）。
...
给定 DAG 中节点 A 直接依赖 B，B 直接依赖 C，那么 A 传递依赖于 C。传递闭包计算所有这种传递依赖关系。

形式化：设 R 为直接依赖关系，传递闭包 R* = R ∪ R² ∪ R³ ∪ ...

**为什么需要**

在融合分析中，如果节点 A 的祖先集合包含节点 B，那么 A 不能与 B 融合（会产生循环）。传递闭包使得这种检查变成 O(1) 的集合查找，而不是每次都做图遍历。

**在 Inductor 中的体现**

`compute_ancestors()` 方法（`scheduler.py` line 3947-3963）利用拓扑排序的性质高效计算传递闭包：

```
对于拓扑排序中的每个节点 n:
    ancestors(n) = { 直接依赖的节点 } ∪ { ancestors(直接依赖的节点) }
```

因为节点按拓扑序处理，所以处理节点 n 时，其所有直接依赖的 ancestors 已经计算完毕。这是一个 O(V+E) 的算法，比朴素的矩阵乘法 O(V³) 高效得多。
...
```mermaid
graph TB
    subgraph "PyTorch 编译栈"
        DYNAMO["torch._dynamo<br/>(字节码追踪)"]
        FX["torch.fx<br/>(图 IR)"]
        AOT["AOTAutograd<br/>(前向/反向分离)"]
        IND["torch._inductor<br/>(后端编译器)"]
    end

    subgraph "Inductor 内部流水线"
        IR_LOW["IR Lowering<br/>(Ch05)"]
        DEP["依赖分析<br/>(Ch06, 本章)"]
        FUSION["算子融合<br/>(Ch07)"]
        CG["代码生成<br/>(Ch08)"]
    end
