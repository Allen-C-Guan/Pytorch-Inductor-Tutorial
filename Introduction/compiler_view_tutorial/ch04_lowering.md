...
编译器中的翻译通常遵循三个步骤：模式匹配（识别源结构）、规则应用（按预定义规则转换）、属性传播（传递 dtype/shape 等元信息）。这与编译器理论中的**语法制导翻译方案（Syntax-Directed Translation, *EaC* Ch.5）** 相似，其中综合属性（如输出 dtype 由输入推导）和继承属性（如 device 从上下文继承）在翻译过程中传播。Inductor 的每个 ATen 算子 Lowering 规则正是一种翻译方案：将源模式（ATen 算子调用）映射到目标模式（Inductor IR 节点构造），同时传播 dtype、device、shape 等属性。
...
这种模式遵循开放封闭原则——添加新算子的 Lowering 只需注册新函数，无需修改翻译框架本身。
...
注：源码行号随版本变化，以下使用函数名作为定位锚点，读者可通过 grep 搜索定位。
...
#### 阶段 5.5：`realize()` —— 从惰性 IR 到具体缓冲区

在 `output()` 阶段，所有输出和被变异的输入都会触发 `StorageBox.realize()`。这是 Lowering 中的关键转折点——将惰性的 IR 节点转换为具名的内存分配。

**What：** `realize()` 将 `StorageBox` 中未物化的数据（如 `Pointwise`、`Reduction`）包装为 `ComputedBuffer`，并注册到 `V.graph` 的 buffer 和 operation 列表中。

**How：** `StorageBox.realize()` 的核心逻辑：

1. 检查 `self.data` 是否已物化（`is_realized_node` 检查是否为 Buffer 类型）
2. 如果未物化，创建 `ComputedBuffer(name=None, layout=FlexibleLayout(...), data=self.data)`
3. 通过 `V.graph.register_buffer()` 分配唯一的缓冲区名称
4. 通过 `V.graph.register_operation()` 注册为计算操作
5. 将 `self.data` 替换为新创建的 `ComputedBuffer`

物化前后的数据流变化：物化前，`StorageBox` 内部是惰性的 `Pointwise`/`Reduction`，可以通过 `make_loader()` 将计算内联到消费者中；物化后，内部变为 `ComputedBuffer`，`make_loader()` 返回 `ops.load(name, index)`，计算不再被内联。

**Why：** realize 是延迟求值的终止点。它为 scheduler 划定了优化边界——未 realize 的计算可以被融合，realize 后的计算成为独立的调度单元。在 `output()` 阶段强制 realize 所有输出，确保最终结果被正确写入内存。
...
**模式 4：规约操作 —— `make_reduction`**

规约操作（如 `sum`、`max`、`argmax`）与逐元素操作有本质区别：它们将某些维度压缩为单一值，输出形状小于输入。`make_reduction()` 是规约操作的 IR 构造工厂：

```python
# lowering.py 中的 make_reduction 工厂函数
def make_reduction(reduction_type: ReductionType, override_return_dtype=None):
    def inner(x, axis=None, keepdims=False, *, dtype=None):
        # 1. 解析规约轴：确定哪些维度被规约、哪些被保留
        kwargs = _make_reduction_inner(x, axis=axis, keepdims=keepdims, ...)
        # 2. 创建 Reduction IR 节点
        result = Reduction.create(reduction_type=reduction_type, input_node=x, **kwargs)
        # 3. 规约操作立即 realize（与 Pointwise 不同）
        result.realize()
        return result
    return inner
```

**与 Pointwise lowering 的关键区别：**

| 维度 | Pointwise Lowering | Reduction Lowering |
|------|-------------------|--------------------|
| IR 节点类型 | `Pointwise` | `Reduction` |
| inner_fn 签名 | `(index) -> value` | `(index, reduction_index) -> value` |
| 输出形状 | 与输入相同 | 被规约的维度消失 |
| 惰性求值 | 默认不 realize | 创建后立即 `realize()` |
| 融合性 | 可与相邻 Pointwise 融合 | 作为融合边界 |

**规约维度的处理：** `_make_reduction_inner()` 函数将输入的 `axis` 参数解析为两组索引：`kept_idx`（保留的维度）和 `reduced_idx`（被规约的维度）。`inner_fn` 接受两个参数——`index` 遍历保留维度，`reduction_index` 遍历被规约维度——并通过索引重组从输入中加载对应位置的值。

**注册方式：** 规约操作通过 `register_lowering` 装饰器注册，内部调用 `make_reduction`：

```python
# 例如 sum 的注册
sum_ = make_reduction("sum")
register_lowering(aten.sum)(sum_)
```

**高阶规约：** 某些操作需要先规约再计算，如 `mean = sum / count`、`var = sum((x - mean)^2) / (n - correction)`。这些通过组合多个 lowering 函数实现，而非单一的 `make_reduction` 调用。
...
Decomposition 在 AOTAutograd 阶段执行，早于 Lowering。其核心是一个全局注册表 `global_decomposition_table`，将 ATen 算子映射到由更简单算子组成的函数：

```python
# torch/_decomp/__init__.py 中的注册表结构
global_decomposition_table = {
    "post_autograd": {},    # AOTAutograd 使用的分解规则
    "pre_autograd": {},     # Autograd 之前的分解规则
    "meta": {},             # 元数据分解规则
}
decomposition_table = global_decomposition_table["post_autograd"]
```

通过 `@register_decomposition` 装饰器注册分解规则：

```python
# torch/_decomp/decompositions.py 中的示例
@register_decomposition(aten.addcmul)
def addcmul_decomp(self, t1, t2, value=1):
    # addcmul(self, t1, t2, value) = self + value * t1 * t2
    return self + value * t1 * t2
```

分解规则将复杂算子转化为更简单的 ATen 算子序列。Lowering 只需为分解后的简单算子提供实现。这意味着 Decomposition 在编译管线中的位置是：

```
Python 代码 → Dynamo → FX Graph → AOTAutograd (Decomposition) → 简化后的 FX Graph → Lowering
```
