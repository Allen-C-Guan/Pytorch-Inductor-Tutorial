...
**前端（Frontend）** 负责理解源代码。它将字符流转换为抽象语法树（AST，Abstract Syntax Tree），进行类型检查，并生成编译器内部的中间表示（IR，Intermediate Representation）。在 Triton 中，前端的工作由 `triton/python/triton/language/` 中的 `semantic.py`（类型检查与语义分析）和 `triton/python/triton/compiler/code_generator.py`（AST 到 TTIR 的转换）共同完成。
