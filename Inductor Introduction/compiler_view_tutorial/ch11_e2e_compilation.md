...
- 第 4 章：Lowering——从 FX Graph 到 Inductor IR
- 第 5 章：图优化
- 第 6 章：依赖分析与调度前置
- 第 7 章：融合策略与循环优化
- 第 8 章：指令选择与代码生成
...
#### 2.1.2 Phase Ordering 与 Phase Interaction

(EaC Section 13.2) **阶段排序问题**是编译器设计中的经典难题：优化阶段的执行顺序会影响最终代码质量，且不存在通用的最优排序。Inductor 采用了固定阶段排序的策略，通过精心设计每个阶段的行为来减少对排序的敏感度。同时，Inductor 将机器无关优化（FX Graph Passes、Decomposition）和机器相关优化（Device-specific Fusion、Triton/C++ Codegen）清晰分离，通过 `get_scheduling_for_device()` 将设备相关逻辑解耦。

阶段交互是 phase ordering 的核心体现——一个阶段的决策会直接影响后续阶段的效果。Inductor 中最显著的 phase interaction 包括：
...
**具体例子**：如果在 Decomposition 阶段将 `torch.nn.GELU` 分解为更基础的运算（而非保持为单一 fused kernel），那么 Fusion 阶段可能有更多融合机会，但也可能错过外部库提供的优化实现。
...
阶段排序理论上可以形式化为组合优化问题，搜索空间为 $O(m!)$ 且目标函数不可微，因此不存在通用的最优解法。Inductor 采用固定阶段排序策略，依赖每个阶段内部的精心设计来降低对排序的敏感度。
...
Inductor 的三个核心设计选择——FX Graph 作为接口、Python 对象作为 IR、Triton 作为 GPU 代码生成目标——都服务于同一个目标：降低编译器的开发和调试成本，同时与 PyTorch 的 Python 生态无缝集成。FX Graph 保留完整的 Python 语义，Python IR 允许直接使用 Python 工具链进行 inspect 和 transform，Triton 则提供了高层 GPU 抽象（自动处理内存合并、寄存器分配等）且生成的 kernel 代码是可读的 Python。（详细讨论见第 1 章）

#### 与其他编译器的比较

与 XLA（HLO IR、静态 shape 为主）和 TVM（Relay IR、显式调度原语）相比，Inductor 的核心差异在于：(1) IR 使用 Python 对象而非文本/二进制格式，(2) 通过 guard 机制而非 shape inference 处理动态形状，(3) GPU 代码生成依赖 Triton 而非直接生成 PTX，使得 autotuning 可以在 Python 层面高效进行。
...
`fx_codegen_and_compile` 中还支持两种特殊的编译模式：

**`_AsyncFxCompile`**：将编译任务提交到后台线程池，主线程立即返回一个 placeholder callable。首次调用时会阻塞等待编译完成，后续调用直接使用缓存。适用于编译时间较长但用户不希望阻塞主线程的场景（如大模型的首次编译）。

**`_ProgressiveFxCompile`**：先用一个快速但优化较少的编译方案（如禁用 autotuning）生成初始代码，让用户尽快开始执行；随后在后台使用完整优化重新编译，编译完成后热替换为优化版本。适用于交互式开发中对首次执行延迟敏感的场景。

这两种模式通过 `torch._inductor.config` 中的 `fx_compile_async` 和 `fx_compile_progressive` 配置项控制，默认均关闭。
...
（注：以下性能数据在具体硬件上测量，仅用于说明相对加速比，实际性能取决于硬件配置）
...
对于更复杂的模型（如 Transformer 的 Multi-Head Attention），Inductor 的优化效果更加显著——QKV 投影可以合并为批量 GEMM，scale + softmax 等逐点操作可以被融合为单个 Triton kernel，而 transpose 和 reshape 通过 View IR 实现零拷贝。读者可以参照上述 MLP 的分析方法，自行追踪 Attention 模块通过管线的完整数据流。
