...
第 8 章负责"生成什么代码"（代码模板和结构），本章负责"代码中的内存操作如何优化"（分配、复用、释放策略）。
...
### 9.2.1 编译器理论（*EaC* Ch.12: Register Allocation）
...
这是最经典的寄存器分配算法，由 Chaitin（1982）提出，后由 Briggs 改进。核心思想是：如果干涉图可以用 K 种颜色着色（K = 物理寄存器数），则所有虚拟寄存器都可以分配到物理寄存器而不溢出。算法通过"简化-选择"循环迭代地移除低度数节点、压栈、再弹出着色；当无法着色时选择代价最低的节点 spill。Coalescing（合并）则消除不必要的 copy 指令——如果两个变量不干涉，可以合并为同一寄存器。

**注意：Inductor 不使用图着色算法进行内存分配，而是采用基于区间调度的池化分配策略（见下文）。** 图着色的完整算法细节见 *EaC* Ch.12 或 Chaitin 1982 原始论文。

#### 线性扫描分配（Linear Scan）

线性扫描是图着色的快速替代方案，由 Poletto & Sarkar（1999）提出。它牺牲分配质量换取编译速度（O(n log n) vs O(n^2)），广泛用于 JIT 编译器。核心思路是按活跃范围的起始时间排序，贪心地分配空闲寄存器，当寄存器用满时选择最远才结束的活跃范围保留、其余溢出。
...
**贪心算法：** 按 f_i（结束时间）升序排列，贪心选择最早结束且不与已选区间重叠的区间。贪心选择最早结束的区间是最优的（证明详见附录或算法教材中的交换论证法）。

**与内存分配的关系：** 如果两个 buffer 的活跃区间不重叠，它们可以共享同一块内存。最大互不重叠子集中的 buffer 数量就是可以共享同一内存位置的 buffer 数量。

#### Sweep-line 算法（用于峰值内存计算）

Sweep-line 算法将每个 buffer 的分配和释放作为事件点，按时间排序后线性扫描，维护当前活跃 buffer 的资源总和，其最大值即为峰值内存。复杂度 O(n log n)（排序）。这正是 Inductor 中 `estimate_peak_memory()`（memory.py line 435）的核心逻辑。
...
> **注：** 传统编译器中 spill 将寄存器值溢出到栈，而 GPU 端没有等价的寄存器-栈区分，此类比并不完美。
...
活跃变量分析使用后向 may 分析框架，通过不动点迭代计算每个程序点的活跃变量集合。详细理论见第 6 章。
...
### IR 层与调度层的映射桥梁：`V.graph.name_to_buffer`

内存管理横跨 IR 层和调度层，两者之间的关键桥梁是 `V.graph.name_to_buffer`（定义于 `graph.py` line 485）。这是一个 `dict[str, ir.Buffer]`，将 buffer 名称映射到 IR 层的 `Buffer` 对象。

**用途 1：跨层类型查询。** 调度器在 `compute_dependencies()` 中需要获取 buffer 的 dtype、是否为别名等信息，这些都通过 `name_to_buffer[name]` 查询 IR 层的 Buffer 对象实现。例如 `scheduler.py` 中 `self.name_to_buf` 就是从 `V.graph.name_to_buffer` 初始化的。

**用途 2：定义操作溯源。** `compute_input_distances()` 中通过 `self.name_to_buf[dep.name].defining_op_name()` 追溯一个 buffer 是由哪个操作定义的。这需要 `name_to_buffer` 提供 buffer 到其定义节点的反向映射。

**用途 3：别名与 mutation 检测。** 当一个 buffer 是另一个 buffer 的 view 时，`name_to_buffer` 中的 `Buffer.get_aliases()` 方法返回别名列表。调度器据此合并别名 buffer 的用户集合，确保所有别名依赖都被正确追踪。

**用途 4：内存规划中的生命周期追踪。** `MemoryPlanningInfoForBuffer`（memory.py line 40）记录的 `succ_nodes` 是通过遍历 `name_to_buffer` 中的用户列表来构建的。buffer 的生命周期（从定义到最后使用）依赖于这些映射表来精确计算。

**设计决策：** `name_to_buffer` 在 `GraphLowering` 的 placeholder 处理阶段逐步填充（`graph.py` line 1065），在整个编译过程中保持一致。它是一个编译期的"符号表"，将名称（字符串）映射到 IR 对象，使得调度层不需要直接持有 IR 节点的引用，降低了层间耦合。
...
### 9.4.2 池分配类型（memory_planning.py）
...
### 9.4.5 整体内存规划器（memory_planning.py）
