...
> 📖 **延伸阅读（具象全景）**
>
> 本章将从**概念层面**铺开编译器理论与 Inductor 的设计哲学——这对理解"为什么"至关重要，但也容易让初学者停留在抽象描述，难以想象"编译器到底产出了什么"。如果你希望先用一个**真实、可对照的例子**把整条管线走一遍，强烈建议先阅读专题文章 **[《一文看穿 Inductor：`torch.compile` 五层编译全记录——循环与寄存器在最末才出生的编译之旅》](inductor_full_tour.md)**。
>
> 它跟着两个极简例子（`exp(A + B)` 广播加法 + `softmax` 归约）走完 Inductor 编译栈的**全部 5 层**（Dynamo → 分解 → Lowering → 调度融合 → Codegen），每一层都配**真实运行的中间产物**——而非手写伪代码：真实的 FX graph、`inner_fn` 符号闭包、融合前后的 IR、`+fusion` 决策日志，直到最终生成的 `@triton.jit` 核。
>
> **推荐阅读方式**：先通读《一文看穿 Inductor》，对"每一层到底产出什么"形成**产物级**的具象心智模型 → 再回到本章，用编译器理论框架为这些产物命名、归类、解释"为什么"。具象的产物能让本章的抽象概念有所锚点，不至于悬空。
